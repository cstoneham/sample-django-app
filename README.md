...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv sampleappbackend` or `python -m venv sampleappbackend-venv`.
...
- [ ] Make sure the virtualenv is activated `workon sampleappbackend` or `source sampleappbackend-venv/bin/activate`.
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `sampleappbackend/backend/sampleappbackend/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `sampleappbackend/backend/sampleappbackend/settings/production.py`
...
- Create a copy of ``sampleappbackend/settings/local.py.example``:  
  `cp sampleappbackend/settings/local.py.example sampleappbackend/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/sampleappbackend/settings/base.py``.
- Adjust ``backend/sampleappbackend/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
